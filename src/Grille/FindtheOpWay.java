...
public class FindtheOpWay extends JFrame implements MouseListener,
		ActionListener {
	/**
	 * 
	 */
	...
	Window win;
	int niveau;
	MyJLabel indication = new MyJLabel(
			Color.BLUE,
			"<HTML>maintenant vous pouvez choisir le obstacles<br><br> que vous voulez <br><br> et c'est à nous de retrouvez le chemin minimal</html>");
	...
	FindtheOpWay(int xd, int yd, int xa, int ya, int largeur, int hauteur,
			int niveau) {
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (x <= tx + lC * largeur
				...
				&& x >= tx
				&& y >= ty
				&& ((x % lC == 0) || (x - 1) % lC == 0 || (x + 1) % lC == 0
						|| (x + 2) % lC == 0 || (x - 2) % lC == 0)
				&& ((y % lC == 0) || (y - 1) % lC == 0 || (y + 1) % lC == 0
						|| (y + 2) % lC == 0 || (y - 2) % lC == 0)) {
			...
			try {
				...
				g.drawImage(img, x / lC * lC - lC / 2, y / lC * lC - lC / 2,
						lC, lC, this);
			} catch (IOException s) {
			}
		}
	...
	public Point trouveMin(ArrayList<Point> c) /*
												 * pour trouver le min d'une
												 * ArrayList
												 */
	{
		int min, test;
	...
	public boolean verif(Point p) /*
								 * pour verifier si notre point n'est pas ub
								 * obstacle et s'il appartient a l'univer du
								 * labyrinth
								 */
	{
	...
	public void ajoutSiPossible(Point successeur) {
		...
		if (index > 0 && test) {
			if (ouvert.get(index).L > successeur.L) /*
													 * le cout du successeur est
													 * inferier a celui qui
													 * existe deja
													 */
			{
				ouvert.remove(ouvert.get(index));/*
												 * on le remplace par le
												 * successeur
												 */
	...
	public void dessineChemin(Graphics g) {
		...
		while (it.hasNext()) {
			...
			/*
			 * tx+z.x*lC<tx+w.x*lC&&g.drawLine(tx+z.x*lC,ty+z.y*lC,tx+w.x*lC,ty+w
			 * .y*lC);
			 */
			try {
				Image img = ImageIO.read(new File("HELLO.jpg"));
				if (ty + z.y * lC < ty + w.y * lC
						|| tx + z.x * lC < tx + w.x * lC)
					g.drawImage(
							img,
							tx + z.x * lC,
							ty + z.y * lC,
							Math.abs(ty + w.y * lC - ty - z.y * lC) / 4
									+ Math.abs(tx + w.x * lC - tx - z.x * lC),
							Math.abs(tx + w.x * lC - tx - z.x * lC) / 4
									+ Math.abs(ty + w.y * lC - ty - z.y * lC),
							this);
				if (ty + z.y * lC > ty + w.y * lC
						|| tx + z.x * lC > tx + w.x * lC)
					g.drawImage(
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (!ouvert.isEmpty()) {
			while (dot.x != xd || dot.y != yd) {
				dot = new P(trouvePoint(dot.x, dot.y).xan, trouvePoint(dot.x,
						dot.y).yan);
				cheminOptimal.add(dot);
			}
