...
public class Window extends JPanel {
	...
	public Window(int largeur, int hauteur, int xd, int yd, int xa, int ya,
			int lC, int niveau) {
		super();
		...
		if (niveau == 1) {
			while (j <= xa) {
				try {
					...
					g.drawImage(img, (j * lC + tx) - lC / 2, (yd * lC + ty)
							- lC / 2, lC, lC, this);
				} catch (IOException s) {
				}
				j = j + 1;
			...
			while (j < ya) {
				try {
					Image img = ImageIO.read(new File("mur3.jpg"));
					g.drawImage(img, ((xd + Math.abs(xd - xa) / 2) * lC + tx)
							- lC / 2, (j * lC + ty) - lC / 2, lC, lC, this);
				} catch (IOException s) {
				}
				j = j + 1;
			}
			...
			while (j <= xa) {
				try {
					Image img = ImageIO.read(new File("mur3.jpg"));
					g.drawImage(img, (j * lC + tx) - lC / 2,
							((ya - 1) * lC + ty) - lC / 2, lC, lC, this);
				} catch (IOException s) {
				}
