...
MyJLabel presentation=new MyJLabel(Color.WHITE,"<html> <blockquote><blockquote><blockquote>Bienvenue dans LabyrinthusOpSolution</blockquote></blockquote></blockquote><br>" +
...
MyJLabel l1x=new MyJLabel(Color.white,"<html>l'abscisse du point de depart<br><br>(il ne faut pas que ca depasse 40)</html>");
MyJLabel l1y=new MyJLabel(Color.white,"<html>l'ordonné du point de depart<br><br>(il ne faut pas que ca depasse 30)</html>");
MyJLabel l2x=new MyJLabel(Color.white,"<html>l'abscisse du point d'arrivé<br><br>(il ne faut pas que ca depasse 40)</html>");
MyJLabel l2y=new MyJLabel(Color.white,"<html>l'ordonné du point d'arrivé<br><br>(il ne faut pas que ca depasse 30)</html>");
...
presentation.setBorder(BorderFactory.createEmptyBorder(120, 20, 20, 20));
...
public void paintComponent(Graphics g){
	...
	g.drawString("ENTER DES COORDONNEES COMPRIS ENTRE 0 ET "+largeur,100,180);
}
...
if(e.getSource()==suiv&&a==2){
	...
	co=new FindtheOpWay(xd,yd,xa,ya,largeur,hauteur,niveauDifficulte.getSelectedIndex());
	if(xa<0||xa>largeur||ya<0||ya>hauteur||xd<0||xd>largeur||yd<0||yd>hauteur){
		...
		g.setColor(Color.WHITE);
		g.setFont(new Font("Serif",Font.BOLD,20));
		g.drawString("veuillez d'abord entrez les coordonnees correcte",100,600);
	}
}
